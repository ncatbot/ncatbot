...
class TestStatus:
    ...
    def test_register_logger_duplicate(self, fresh_status):
        ...
        assert loggers.count("dup_logger") if isinstance(loggers, list) else "dup_logger" in loggers
    ...
    def test_thread_safety_register_logger(self, fresh_status):
        ...
        threads = [threading.Thread(target=register, args=(f"logger_{i}",)) for i in range(10)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
