...
class TestServiceConfiguration:
    ...
    @pytest.mark.asyncio
    async def test_service_receives_config(self):
        """测试服务接收配置参数"""
        manager = ServiceManager()
        
        manager.register(SlowService, delay=0.05)
        
        service = await manager.load("slow_service")
        
        assert service.delay == 0.05
        assert service.load_time is not None
        assert service.load_time >= 0.05
...
class TestDynamicServiceManagement:
    """动态服务管理测试"""
    
    @pytest.mark.asyncio
    async def test_load_unload_reload_cycle(self):
        """测试加载-卸载-重新加载循环"""
        manager = ServiceManager()
        load_order = []
        close_order = []
        ...
        # 加载
        await manager.load("service_a")
        assert "service_a" in manager._services
        ...
        # 验证生命周期
        assert load_order == ["service_a", "service_a"]
        assert close_order == ["service_a"]
    ...
    async def test_get_service_by_name(self):
        ...
        # 通过内置属性访问（如果定义了）
        # 这里通过 _services 访问
        service = manager._services.get("service_a")
...
class TestServiceStateQuery:
    """服务状态查询测试"""
    
    @pytest.mark.asyncio
    async def test_service_is_loaded_property(self):
        """测试 is_loaded 属性"""
        manager = ServiceManager()
        manager.register(ServiceA, load_order=[], close_order=[])
    ...
    @pytest.mark.asyncio
    async def test_service_repr(self):
        """测试服务的字符串表示"""
        manager = ServiceManager()
        manager.register(ServiceA, load_order=[], close_order=[])
