...
class DecoratorValidator:
    ...
    def validate_function_decorators(func: Callable):
        ...
        # 获取装饰器信息 (Spec 对象列表)
        options: List[OptionSpec] = getattr(func, '__command_options__', [])  # List[OptionSpec]
        params: List[ParameterSpec] = getattr(func, '__command_params__', [])   # List[ParameterSpec]
        groups: List[OptionGroupSpec] = getattr(func, '__command_option_groups__', [])  # List[OptionGroupSpec]
