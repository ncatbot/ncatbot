...
class HelpGenerator:
        ...
        if param.is_multi_type():
            ...
            for type_obj in union_type.types:
                ...
                if examples:
                    ...
                    lines.append(f"{self.indent}{self.indent}{type_name}示例: {examples_str}")
    ...
    def _format_option(self, option: OptionSpec) -> List[str]:
        ...
        if option.needs_value():
            type_name = option.value_type.__name__ if hasattr(option.value_type, '__name__') else str(option.value_type)
