...
1. 确认所有代码已合并到目标分支
2. 确认测试通过（`python -m pytest`）
3. 根据 commit 类型确定版本递增策略（major/minor/patch）
4. 更新 `pyproject.toml` 中的 `version` 字段
   - 正式版：`"5.0.0"`
   - 预发布版：`"5.0.0rc1"`、`"5.0.0a1"`、`"5.0.0b1"`
...
目标：将 `examples/`、`skills/`、`docs/` 打包为一个 zip 文件，供用户下载参考。
